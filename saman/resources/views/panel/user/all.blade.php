                            ...
                            <div class="card-body table-responsive p-0">
                                <table class="table table-hover table-bordered table-striped">
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$product->email}}</td>
                                        <td>
                                        @if($product->level == 'admin')
                                                <a href="{{route('users.setuser',['id'=>$product->id])}}">
                                            <button class="btn btn-block btn-outline-danger">   <i class="right fa fa-user-secret "></i></button>
                                                </a>
                                            @else
                                                <a href="{{route('users.setadmin',['id'=>$product->id])}}">
                                                <button class="btn btn-block btn-outline-success">   <i class="right fa fa-user "></i></button>
                                                </a>
                                            @endif
                                        </td>
                                        <td>
                                            <form action="{{ route('users.destroy'  , ['id' => $product->id]) }}" method="post">
                                                {{ method_field('delete') }}
                                                {{ csrf_field() }}

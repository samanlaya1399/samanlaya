                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">جدول مطالب</h3>
                            ...
                            <div class="card-body table-responsive p-0">
                                <table class="table table-hover table-bordered table-striped">
                                    ...
                                    <tr>
                                        <td>{{$product->id}}</td>
                                        <td>{{$product->title}}</td>
                                        <td>      @foreach($product->categories->pluck('name') as $key=>$name )
                                                {{$name.' ، '}}
                                            @endforeach</td>
                                        <td>
                                            <form action="{{ route('articles.destroy'  , ['id' => $product->id]) }}" method="post">
                                                {{ method_field('delete') }}
                                                {{ csrf_field() }}
                                                <div class="btn-group btn-group-xs">
                                                    <a href="{{ route('articles.edit' , ['id' => $product->id]) }}"  class="btn btn-primary">ویرایش</a>

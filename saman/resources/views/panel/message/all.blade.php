                            ...
                            <div class="card-body table-responsive p-0">
                                <table class="table table-hover table-bordered table-striped">
                                    ...
                                    <tr>
                                        ...
                                        <td> {{$product->body}}    </td>
                                        <td> 
                                        @if($product->subject == 1)  
                                        انتقادات و پیشنهادات
                                        @elseif($product->subject == 2)
                                        گزارش تخلف
                                        @elseif($product->subject == 3)
                                        درخواست همکاری
                                        @endif
                                        ...
                                        <td>
                                            <form action="{{ route('message.destroy'  , ['id' => $product->id]) }}" method="post">
                                                {{ method_field('delete') }}
                                                {{ csrf_field() }}
                                                <div class="btn-group btn-group-xs">
                                                    <a href="{{ route('message.set.approved' , ['id' => $product->id]) }}"  class="btn btn-success">تایید</a>

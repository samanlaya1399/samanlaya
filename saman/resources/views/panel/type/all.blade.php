                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">لیست نوع محصولات</h3>
                            ...
                            <div class="card-body table-responsive p-0">
                                <table class="table table-hover table-bordered table-striped">
                                    ...
                                    <tr>
                                        <td>{{$product->id}}</td>
                                        <td>{{$product->title}}</td>
                                        <td>{{$product->grouping->title}}</td>
                                        <td>{{$product->description}}</td>
                                        <td>
                                            <form action="{{ route('type.destroy'  , ['id' => $product->id]) }}" method="post">
                                                {{ method_field('delete') }}
                                                {{ csrf_field() }}
                                                <div class="btn-group btn-group-xs">
                                                    <a href="{{ route('type.edit' , ['id' => $product->id]) }}"  class="btn btn-primary">ویرایش</a>

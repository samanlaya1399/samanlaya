                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            ...
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form class="form-horizontal" action="{{ route('property.update',['id'=>$property->id]) }}" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                {{ method_field('PATCH') }}
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="exampleInputEmail1">عنوان ویژگی</label>
                                        <input type="text" name="title" class="form-control" id="exampleInputEmail1" placeholder="عنوان را وارد کنید" value="{{$property->title}}">
                                    </div>
                                    <div class="form-group">
                                        <label for="exampleInputEmail1">توضیحات ویژگی</label>
                                        <textarea rows="5" class="form-control" name="description" id="description" placeholder="توضیحات را وارد کنید">{{ $property->description }}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <div class="col-sm-12">
                                            <label for="category">نوع محصول </label>
                                            <select name="type_id" id="category" class="form-control select" title="بخش مورد نظر را انتخاب کنید"  data-live-search="true" >
                                                @foreach($types as $group)
                                                    <option value="{{$group->id}}" @if($group->id == $property->type->id) selected @endif >{{$group->title}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
